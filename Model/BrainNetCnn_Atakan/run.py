...
from Models.BrainNetCnn.code.model import Model
from Models.SVM.code.util import corrcoef
...
def calculateFC(timeseries):
    ...
    return torch.tensor(np.array(FCs))
    ...
    if(taskType == "classification"):
        ...
        for epoch in range(nOfEpochs):
                ...
                for i, data in enumerate(tqdm(dataLoader, ncols=60, desc=f'fold:{fold} epoch:{epoch}')):
                    ...
                    time1 = time.time()
                    xTrain = calculateFC(data["timeseries"]).unsqueeze(dim=1) # (batchSize, 1, N, N)
...
def test(model, dataset, taskType, fold):
    ...
    if(taskType == "classification"):
        ...
        for i, data in enumerate(tqdm(dataLoader, ncols=60, desc=f'Testing fold:{fold}')):
            ...
            xTest = calculateFC(data["timeseries"]).unsqueeze(dim=1)
            yTest = data["label"]
        ...
        for i, data in enumerate(tqdm(dataLoader, ncols=60, desc=f'Testing fold:{fold}')):
            ...
            xTest = calculateFC(data["timeseries"]).unsqueeze(dim=1) # (batchSize, 1, N, N)
...
def run_brainNetCnn(hyperParams, datasetDetails, device="cuda:0", save=False):
